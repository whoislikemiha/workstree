...
func (s *Suggestion) Render() string {
	...
	b.WriteString("# committing (create a throwaway worktree, run these steps or `workstree init`).\n")
	b.WriteString("# If using workstree, `init` executes copy/setup/ready and `teardown` executes teardown.\n\n")

	b.WriteString("# setup lists shell commands to run in a fresh worktree, in order.\n")
	b.WriteString("# Use it for dependency installs, codegen, migrations, or starting dev runtimes.\n")
	if len(s.Setup) > 0 {
		b.WriteString("setup = [\n")
	...
	b.WriteString("\n# teardown lists shell commands to run before removing a worktree.\n")
	b.WriteString("# Use it to stop/delete resources owned by this worktree: containers, DB volumes, runtimes.\n")
	b.WriteString("# teardown = [\"./scripts/worktree-runtime down -v\"]\n")

	b.WriteString("\n# copy lists untracked files/directories to copy from the source checkout.\n")
	b.WriteString("# Tracked files already come with the worktree; this list is usually secrets.\n")
	b.WriteString("# Entries are relative paths only; missing sources are skipped. Review carefully.\n")
	if len(s.Copy) > 0 {
		b.WriteString("copy = [\n")
	...
	b.WriteString("\n# ready is a smoke-check shell command run after setup; nonzero = NOT ready.\n")
	b.WriteString("# Prefer a fast build/typecheck/health check over a full test suite.\n")
	if s.Ready != "" {
